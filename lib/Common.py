  ...
  def getImageUrl(self, search_item, total_num):
    ...
    while i < total_num:
      ...
      for j in range(len(data["items"])):
        img_dict["thumbnail"] = data["items"][j]["image"]["thumbnailLink"]
        img_dict["link"] = data["items"][j]["link"]
        #img_dict.append(data["items"][j]["image"]["thumbnailLink"])
        #img_dict.append(data["items"][j]["link"])
      i=i+10
    return img_dict
